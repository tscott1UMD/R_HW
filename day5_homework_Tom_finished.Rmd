...
#alpha changes the transparency of density plots if you fill with a variable 
...
ggplot(data, aes(x)) + geom_histogram() + geom_vline(xintercept = 50, color = "red")
```
...
+ scale_fill_brewer(palettte = "Set1")  # see colorbrewer2.org
```
...
scatter
loess(formula = S0SROAGG ~ S0SROINC, data = pathbase)
ggplot(x = pathbase$S0SROAGG, y = pathbase$S0SROINC)
